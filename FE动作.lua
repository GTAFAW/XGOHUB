...
RunService.Heartbeat:Connect(function()
    if player.Character and player.Character.Humanoid.RigType == Enum.HumanoidRigType.R6 then
        ...
        if errorMsg.Visible then
            errorMsg.ErrorText.Text = "仅支持R15，R6无法使用"
        end
    end
end)
...
local function applyAnimation(animationData)
    ...
    if not animate or not humanoid then
        getgenv().Notify({
            Title = '7yd7 | 动画错误',
            Content = '未找到Animate（动画）或Humanoid（人形对象）',
            Duration = 3
        })
        return
    end
...
local function fetchAllEmotes()
    ...
    getgenv().Notify({
        Title = '7yd7 | 表情动作',
        Content = "加载成功！表情动作总数：" .. totalEmotesLoaded,
        Duration = 5
    })
...
if Changepage then
    Changepage.MouseButton1Click:Connect(function()
        ...
        if currentMode == "emote" then
            ...
            getgenv().Notify({
                Title = '7yd7 | 动画',
                Content = '已切换至表情动作 > 动画模式',
                Duration = 3
            })

         getgenv().Notify({
            Title = '7yd7 | 动画',
            Content = "点击任意动画（点击动画图标操作）",
            Duration = 5
        })
        else
            currentMode = "emote"
